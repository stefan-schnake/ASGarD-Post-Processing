...
% To integrate out kth dimension
%   g(x_1,...,x_{k-1},x_{k+1},...,x_N) = 
%           int f(x_1,...,x_k,...x_N)q(x_k) dx_k
% use command
% >> G = ttv(F,asgdata(k).weights.*q(asgdata(k).nodes),k);

% To determine the L^2 norm of two tensor functions
%    ||f-g||_{L^2} 
% use command where F,G are d-tensors
% >> nrm2 = sqrt(ttv((F-G).^2,{asgdata(:).weights},1:d))

%%% Plotting Tensors

% To plot a 2d tensor F:
% >> coords = get_tensor_coords(asgdata);
% >> surf(coords{1},coords{2},F.data,'EdgeColor','none'); view([0 90])
